  ...
  return (
    <main className="mx-auto flex min-h-[100dvh] w-full max-w-[480px] flex-col overflow-x-hidden px-3 pb-24 pt-3">
      <header className="sticky top-0 z-40 -mx-3 border-b border-white/10 bg-ink/90 px-3 py-3 backdrop-blur-glass">
        <div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-xs uppercase tracking-[.18em] text-cyan-200/70">{t.subtitle}</p><h1 className="truncate text-2xl font-black leading-tight">{t.title}</h1></div><div className="flex shrink-0 gap-2"><IconButton icon={Crown} label={t.vip} onClick={() => setActiveTab("vip")} tone="gold" /><IconButton icon={Bell} label="Notify" onClick={() => setToast(t.notifications)} /></div></div>
...
function StatsStrip({ t }: { t: Copy }) { return <section className="mt-3 grid grid-cols-3 gap-2"><MiniStat title={t.rtp} value={`${economyStats.targetRtp}%`} /><MiniStat title={t.houseEdge} value={`${economyStats.houseEdge}%`} /><MiniStat title={t.jackpot} value={`$${economyStats.jackpotPool}`} /></section>; }
