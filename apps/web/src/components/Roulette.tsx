...
export function Roulette({ isOpen, items, result, caseName = "Gift Case", price = 0, balance = 0, onClose, onDeposit, onSell }: Props) {
        ...
        <motion.div className="fixed inset-0 z-50 grid place-items-end bg-black/75 p-2 backdrop-blur-md sm:place-items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.div className="max-h-[92dvh] w-full max-w-[380px] overflow-y-auto rounded-[14px] border border-white/10 bg-[#151515] p-4 shadow-[0_24px_100px_rgba(0,0,0,.75)]" initial={{ y: 36, scale: 0.98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 36, scale: 0.98 }}>
            <div className="flex items-center justify-between">
              <div className="w-10" />
              <h2 className="truncate text-center text-lg font-black">{caseName}</h2>
            ...
            <div className="relative mt-3 overflow-hidden rounded-[10px] bg-[#101010] px-4 py-2">
              ...
              <div className="absolute inset-x-1/2 bottom-0 h-1 w-24 -translate-x-1/2 rounded-full bg-blue-500 shadow-[0_0_24px_rgba(24,119,242,.8)]" />
            ...
            {notEnoughFunds ? (
              ...
              <motion.div className={`rarity-${result.rarity.toLowerCase()} mt-8 rounded-[12px] border border-white/10 bg-white/[.06] p-4 text-center shadow-mythic`} initial={{ opacity: 0, y: 16, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.45 }}>
                <div className="mx-auto grid h-20 w-20 place-items-center rounded-[12px] bg-black/30" style={{ color: "var(--rarity)" }}><Gift size={38} /></div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2"><button onClick={onSell} className="h-11 rounded-[10px] border border-white/10 bg-white/[.06] text-sm font-bold">Sell</button><button onClick={onClose} className="h-11 rounded-[10px] bg-[#1683ff] text-sm font-black text-white">Keep</button></div>
              </motion.div>
            ) : null}

            <div className="mt-12">
              <p className="px-2 text-sm font-bold text-white/45">Possible prizes:</p>
              <div className="mt-4 grid grid-cols-3 gap-3">{items.map((item) => <PrizeTile key={item.id} item={item} />)}</div>
            </div>
          </motion.div>
        </motion.div>
      ) : null}
